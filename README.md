...
uv run pipt run external ginandjuice ./scope.txt -v
...
uv run pipt run external ginandjuice ./scope.txt --config pipt.toml --set oast=on
```
...
uv run pipt run external ginandjuice ./scope.txt --observe     # terminal 2
```
...
Views of the external pipeline's flow — **all auto-generated from the code** by
`pipt.pipelines.external.flowmeta` and regenerated by a hook on any change under `src/pipt/pipelines/`
