...
read wl_global/<role>.txt by role. Resolution per role, in priority order:
  ...
  2. explicit  — env PTFLOW_WL_<ROLE> = /abs/path/to/list.txt
...
Search dirs = env PTFLOW_WORDLISTS (':'-separated) ++ common collection locations. This keeps
...
def search_dirs() -> list[Path]:
    """Wordlist base dirs: env PTFLOW_WORDLISTS (':'-sep) prepended to common locations,
    filtered to those that exist."""
    env = os.environ.get("PTFLOW_WORDLISTS", "")
...
def _resolve(role: str) -> Path | None:
    ...
    override = os.environ.get(f"PTFLOW_WL_{role.upper()}")
