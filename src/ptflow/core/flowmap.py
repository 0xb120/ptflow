...
it stays generic. The shared driver `ptflow.core.flowdocs` pairs each pipeline's stages with the
`MapSpec` it exposes via `flowmap_spec()` and writes the `docs/<name>-pipeline-*` files.
"""
...
def render(stages: Sequence[Stage], spec: MapSpec, *, generator: str = "ptflow.core.flowdocs") -> str:
...
{body}
<footer>generato da <b>{generator}</b> · non modificare a mano · struttura derivata dagli Stage objects</footer>
</div>
</body>
</html>
"""
