...
regenerates the map (the same hook that keeps `pipeline-flow.html` in sync).
...
if TYPE_CHECKING:
    ...
    from pipt.core.flowmap import MapSpec, StepMeta
...
def render_html(stages: Sequence[Stage], spec: MapSpec) -> str:
    """Full self-contained pan/zoom HTML (Mermaid via CDN). Deterministic."""
    graph = mermaid_graph(stages, spec, rich=True)
    return _HTML.replace("__TITLE__", _mesc(spec.title)).replace(
        "__MERMAID_GRAPH_JSON__", json.dumps(graph))


def render_markdown(stages: Sequence[Stage], spec: MapSpec) -> str:
    """The same flowchart as a Markdown ```mermaid block (renders on GitHub). Deterministic."""
    graph = mermaid_graph(stages, spec, rich=False)
    return _MD.replace("__TITLE__", spec.title).replace("__MERMAID_GRAPH__", graph)
